...
export class MainScene extends g.Scene {
	...
	public labelStage: g.Label;
	public labelLevel: g.Label;

	constructor(param: g.SceneParameterObject) {
		param.assetIds = [
			"img_numbers_n", "img_numbers_n_red", "title", "start", "finish", "score", "time",
			"coin","map","star","stage","select","clear","hand",
			"config", "volume", "test", "glyph72", "number_w", "number_b", "number_p",
			"se_start", "se_timeup", "bgm", "se_clear", "se_move", "se_miss","se_select","se_coin"];
		super(param);
		...
		this.loaded.add(() => {
			...
			//スコア
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["score"], x: 505, y: 5, height: 32 }));
			let score = 0;
			...
			//何問目か
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["score"], x: 430, y: 5, height: 32, srcY: 32 }));
			this.labelStage = new g.Label({
				scene: this,
				x: 360,
				y: 4,
				width: 32 * 2,
			...
			uiBase.append(this.labelStage);
			
			//レベル
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["score"], x: 380, y: 40, height: 32, srcY: 64 }));
			this.labelLevel = new g.Label({
				scene: this,
				x: 455,
				y: 38,
				fontSize: 32,
			...
			uiBase.append(this.labelLevel);

			//タイム
			uiBase.append(new g.Sprite({ scene: this, src: this.assets["time"], x: 3, y: 10 }));
			const labelTime = new g.Label({ scene: this, font: numFont, fontSize: 32, text: "70", x: 43, y: 13 });
			...
			//リセットボタン
			const btnReset = new Button(this, ["リセット"], 500, 290, 130);
			if ((typeof window !== "undefined" && window.RPGAtsumaru) || isDebug) {
			...
			//ランキングボタン
			const btnRanking = new Button(this, ["ランキング"], 500, 230, 130);
			if ((typeof window !== "undefined" && window.RPGAtsumaru) || isDebug) {
